...
function renderDialog() {
  return render(
    <AddToolDialog
      trigger={<button type="button">도구 추가</button>}
    />
  )
}
...
describe("AddToolDialog", () => {
  it("opens dialog when trigger clicked", async () => {
    ...
    expect(screen.getByText("MCP 서버를 등록하거나 커스텀 도구를 직접 정의하세요.")).toBeInTheDocument()
  })
  ...
  it("submits MCP server form when filled and clicked", async () => {
    ...
    // Fill in MCP form
    await user.type(
      screen.getByPlaceholderText("Google Workspace MCP"),
      "My MCP Server"
    )
    await user.type(
