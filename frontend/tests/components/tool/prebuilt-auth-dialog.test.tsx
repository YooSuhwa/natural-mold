...
describe('PrebuiltAuthDialog', () => {
  ...
  it('opens dialog with tool info when trigger clicked', async () => {
    ...
    expect(
      screen.getByText(
        '이 도구를 사용하려면 API 키를 설정하세요. 네이버 개발자센터에서 발급받을 수 있습니다.',
      ),
    ).toBeInTheDocument()
  })
  ...
  it('shows existing config indicator when tool has auth_config', async () => {
    ...
    renderDialog(googleSearchTool)
    await user.click(screen.getByText('키 설정'))
    expect(screen.getByText('API 키가 설정되어 있습니다')).toBeInTheDocument()
  })
  ...
  it('shows no fields for unknown provider', async () => {
    ...
    const user = userEvent.setup()
    render(
      <PrebuiltAuthDialog tool={unknownTool} trigger={<button type="button">키 설정</button>} />,
    )
    await user.click(screen.getByText('키 설정'))
  ...
  it('shows Google workspace fields for Gmail tool', async () => {
    ...
    const user = userEvent.setup()
    render(<PrebuiltAuthDialog tool={gmailTool} trigger={<button type="button">키 설정</button>} />)
    await user.click(screen.getByText('키 설정'))
  ...
  it('shows webhook URL field for Google Chat tool', async () => {
    ...
    const user = userEvent.setup()
    render(<PrebuiltAuthDialog tool={chatTool} trigger={<button type="button">키 설정</button>} />)
    await user.click(screen.getByText('키 설정'))
