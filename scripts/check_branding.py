#!/usr/bin/env python3
"""Branding/license guard.

Verifies the Moldy codebase does not include configured third-party brand
identifiers, package imports, or asset hashes that would violate the current
branding policy. Run as a CI gate.

Checks:
1. Configured forbidden identifier patterns in source trees
2. Configured forbidden package prefixes in manifests
3. Configured forbidden asset SHA-256 hashes in `frontend/public/` + `frontend/src/assets/`
...
FORBIDDEN_IDENTIFIER_PATTERNS: tuple[re.Pattern[str], ...] = ()
...
# Files whose contents are governed separately from the source/doc scan.
WHITELIST_PATHS = {
    "PLAN.md",
    "CHECKPOINT.md",
    "AUDIT.log",
...
FORBIDDEN_NPM_SCOPE_PREFIXES: tuple[str, ...] = ()
FORBIDDEN_PYPI_PREFIXES: tuple[str, ...] = ()
...
def _scan_identifier(violations: list[str]) -> None:
    if not FORBIDDEN_IDENTIFIER_PATTERNS:
        return
    ...
    for rel in PACKAGE_JSON_FILES:
        ...
        for section in ("dependencies", "devDependencies", "peerDependencies", "optionalDependencies"):
            for pkg in (data.get(section) or {}):
                for prefix in FORBIDDEN_NPM_SCOPE_PREFIXES:
...
def main() -> int:
    ...
    if violations:
        ...
        print(
            "Review scripts/check_branding.py for the current branding policy.",
            file=sys.stderr,
        )
        return 1

    print("Branding check PASSED — no configured identifiers, packages, or assets found.")
    return 0
