...
4. **local-first, S3/MinIO-ready로 간다.**  
   MVP는 local disk로 충분하다. 단 storage interface를 먼저 만들고 API는 storage backend에 의존하지 않게 설계한다.
...
`run_id`별 path unique를 MVP 기준으로 잡는다. 같은 `report/final.md`가 다른 실행에서 다시 생성되는 경우 별도 artifact로 보여주고, UI에서 실행 단위로 그룹화한다. 추후 "같은 logical path를 conversation-level 문서로 version merge"하는 정책은 별도 UX 결정 후 확장한다.
...
MVP의 최적 경로는 `streaming.py`의 기존 `emit()`를 그대로 쓰는 것이다. `execute_in_skill` tool result 직후 artifact delta를 ingest하고 `file_event`를 emit하면 broker resume, trace persistence, `message_events` 저장이 자동으로 따라온다.
...
### Milestone 6: Hardening and share
...
### Milestone 7: Optional near-real-time
...
MVP는 tool 종료 후 delta 감지이므로 partial write 문제가 적다. near-real-time polling 단계에서는 size/mtime이 안정화된 뒤 `ready`로 전환하는 debounce가 필요하다.
...
Office, PPT, CAD preview는 파일 타입별 dependency와 변환 서버가 필요하다. MVP 범위에 넣지 않고 provider registry만 먼저 마련한다. 이후 필요한 포맷부터 개별 provider로 붙인다.
...
1차 PR의 목표는 "생성된 파일이 우측 패널에 뜨고, 새로고침 후에도 목록과 preview가 유지된다"까지로 잡는다.
...
- SSE `file_event`
- artifact list/content/download API
...
이 범위가 가장 Moldy답다. 현재 제한된 `execute_in_skill` 모델을 유지하면서도 사용자가 체감하는 artifact 경험을 크게 개선하고, 나중에 sandbox나 MinIO가 필요해졌을 때 갈아엎지 않고 확장할 수 있다.
...
- `backend/app/services/artifact_service.py`  
  output dir snapshot/diff, DB manifest/version creation, content/download helpers.
- `backend/app/routers/artifacts.py`  
  authenticated conversation artifact APIs.
...
- `frontend/src/lib/api/artifacts.ts`  
  Fetch list/content/download metadata.
...
PostgreSQL migration should add `CHECK (status IN (...))` and `CHECK (storage_provider IN (...))`. SQLite tests can rely on SQLAlchemy model plus service validation.
...
```python
    # Agent/runtime generated artifacts. MVP uses local disk; S3/MinIO can be
...
MVP local backend writes canonical copies to `settings.artifact_storage_dir`. Routers should never serve directly from `data/conversations/{conversation_id}` once an artifact row exists.
...
`ingest_changed_files()` must upsert by `(conversation_id, assistant_msg_id, logical_path)`. If an artifact exists and `sha256` changed, create `ArtifactVersion(version_number=max+1)`, update `current_version_id`, `sha256`, `size_bytes`, `updated_at`, and emit `op="updated"`. If not exists, create artifact + version and emit `op="created"`.
...
After `yield emit(event_names.MESSAGE_START, start_data)` and before `agent.astream(...)`, prepare the recorder. If prepare fails, emit an `error` only if the stream cannot proceed. Recommended MVP behavior is fail-open with a log, because artifact panel failure must not block chat.
...
Do not put artifact detection inside `_create_skill_execute_tool()` for MVP. That closure currently validates skill slug, prepares env, runs subprocess, redacts credentials, and appends `OUTPUT_FILES`. Keeping artifact ingest outside the tool avoids DB/session coupling inside tool execution.
...
Provider matching rules for MVP:
...
```bash
cd backend
uv run pytest tests/test_artifacts_router.py -q
```
...
```bash
cd backend
uv run pytest tests/test_artifacts_router.py tests/test_conversations_router.py -q
```
