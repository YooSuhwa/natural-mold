...
| `finalize_skill` | 확정: 검증 재실행 → `claim_for_confirming` → 드래프트 zip → skills row + 리비전 | zip은 **워크스페이스 디스크 기반** `build_workspace_zip_bytes`(`skill_draft_workspace.py` → `build_skill_zip_bytes_from_dir`, Phase 1.5) — text 어댑터가 싣지 못하는 바이너리 asset을 바이트 그대로 포함(`inputs/`·`evals/` 제외, symlink skip). REST `/confirm`은 게시된 `draft_package` 계약 유지(text zip). 생성: `create_package_skill`(전체 zip 방어 재통과, `service.py:136-188`) + `unique_skill_slug`. 개선: `lock_skill_for_mutation` → `base_content_hash` 검사 → `SOURCE_SKILL_CHANGED` → `replace_skill_storage`(`skill_builder_package_storage.py:22-42`). 리비전 `builder_create/builder_improvement`(`skill_revision_service.py:31-74`). secret scan(`marketplace.secret_scan`)은 finalize 전 필수 게이트. zip 상한 초과 등 packager 가드 실패는 `PACKAGE_INVALID`로 claim 해제 후 보고 | **항상 승인 카드** (세션 동의 불가) |
...
- 자동 첫 메시지 (Phase 1.5): 빌더 화면 첫 진입 시 다이얼로그의 `user_request`가 자동으로 첫 사용자 메시지로 발화된다 (`skill-builder-auto-request.tsx` — composerHint 슬롯에서 thread append). 가드: 세션 `active` + envelope 해결 + 대화 0건 + run 이력 없음(리로드/중단 런 재전송 방지). create/improve 공통.
...
- 스튜디오 5탭 IA/목록 표/벌크/버전 diff/연결 카운트 (Phase 2) — **✅ 출시됨:
  `skill-studio-phase2-studio-spec.md` (설정 탭을 더해 6탭 IA로 확장)**
