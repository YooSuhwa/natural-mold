# Architecture Decision Records (ADR) Index

> Last updated: 2026-06-07. Status values below reflect current source state,
> not the original proposal date.

| ADR | 제목 | 상태 | 날짜 |
|-----|------|------|------|
| ADR-001 | Deep Agent 엔진 교체 | 구현됨 | 2026-04-05 |
| ADR-002 | Checkpointer 기반 대화 관리 | 승인됨 | 2026-04-06 |
| ADR-003 | [스킬 + 메모리 전환](adr-003-skills-memory.md) | 승인됨 | 2026-04-06 |
| ADR-004 | [M4 정리 — Creation Agent + Trigger + Streaming](adr-004-m4-cleanup.md) | 승인됨 | 2026-04-06 |
| ADR-005 | [Builder/Assistant 아키텍처](adr-005-builder-assistant.md) | 구현됨 | 2026-04-07 |
| ADR-007 | [Credentials field_keys 캐시 컬럼](adr-007-credentials-field-keys-cache.md) | 승인됨 | 2026-04-17 |
| ADR-008 | [Connection 엔티티 — Credential 바인딩 통합](adr-008-connection-entity.md) | 폐기 (ADR-009로 승계) | 2026-04-18 |
| ADR-009 | [Credential / Tools / Skills 그린필드 리라이트](adr-009-greenfield-credentials.md) | 승인됨 | 2026-04-29 |
| ADR-010 | [UI 토큰 + DialogShell 비주얼 스펙](ADR-010-ui-tokens-and-dialog-shell.md) | 승인됨 | 2026-04-30 |
| ADR-011 | [SSE Stream Resume (W3-out)](adr-011-sse-stream-resume.md) | 구현됨 | 2026-05-03 |
| ADR-012 | [HiTL 미들웨어 마이그레이션](adr-012-hitl-middleware-migration.md) | 승인됨 | 2026-05-04 |
| ADR-013 | [Service-side LLM Key from Credentials](adr-013-service-llm-key-from-credentials.md) | 승인됨 | 2026-05-06 |
| ADR-014 | [Chat Model Factory — Provider Quirks 분리](adr-014-chat-model-factory-strategy.md) | 승인됨 | 2026-05-08 |
| ADR-016 | [멀티유저 인증 (HttpOnly Cookie + JWT + super_user)](adr-016-multiuser-auth.md) | 승인됨 | 2026-05-09 |
| ADR-017 | [Marketplace Resources (Skill/MCP/Agent 공유 레이어, Phase 1: Skill)](adr-017-marketplace-resources.md) | Skill Phase 1 구현됨, MCP/Agent 확장 대기 | 2026-05-18 |
| ADR-018 | [storage_path 상대화 (worktree 데이터 손실 재발 방지)](adr-018-relative-storage-path.md) | 승인됨 | 2026-05-23 |
| ADR-019 | [System LLM Settings (역할별 모델 선택 + base_url 주입)](adr-019-system-llm-settings.md) | 승인됨 | 2026-05-26 |
