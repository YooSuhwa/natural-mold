...
- 신규 transitive `langchain-protocol 0.0.15`. 앱 코드 수정 0건.

### PR #92 — pyright baseline 정리 (36/8 → 0/0)
- **실제 버그 2건 수정**:
  - `AgentResponse.last_used_at` 필드 누락 → Pydantic이 silently drop 중. 사이드바 정렬이 `updated_at` 폴백으로만 동작하던 잠재 버그 해소
  - alembic m7/m9의 `app.services.encryption` dead import → fresh DB 롤업 깨짐 수정
- 앱 코드 Optional/Literal 좁히기 16건, 테스트 타입 좁히기 20건, warnings 8건
...
4. **CollapsiblePill 통일 컴포넌트** — loading/success/error/cancelled 4상태, 1일, 의존성 0
5. **MarkdownContent 강화** — mermaid + katex + remarkBreaks, 1일
...
- **타입 좁히기**: `state.get("X")` 결과는 변수 캐시 후 좁힘 (pyright는 재호출 시 narrowing 잃음). DB `db.get(Model, id)`는 `assert is not None`으로 좁히기. `usage_metadata` 같은 TypedDict는 `cast()` 사용. alembic은 `sa.TextClause` (canonical)
