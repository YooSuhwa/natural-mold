...
def _drop_table_if_exists(name: str) -> None:
    ...
    Uses ``CASCADE`` on PostgreSQL so any lingering FK from a sibling table
    we have not yet dropped (autogenerated FK names that escape
    ``_drop_constraint_if_exists``) is severed automatically. SQLite ignores
    CASCADE but enforces FKs only when ``PRAGMA foreign_keys = ON`` is set.
    """
    ...
    if name not in inspector.get_table_names():
        return
    if bind.dialect.name == "postgresql":
        op.execute(sa.text(f'DROP TABLE "{name}" CASCADE'))
    else:
        op.drop_table(name)
...
def upgrade() -> None:
    ...
    #    populated; the greenfield m18 wipes them all. ``connections`` must
    #    drop before ``credentials`` because ``connections.credential_id`` FK
    #    references it (autogenerated constraint name escapes the explicit
    #    drop above on some DBs).
    for table in (
        "agent_tools",
        "agent_skills",
        "mcp_tools",
