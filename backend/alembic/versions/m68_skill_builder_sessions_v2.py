...
revision = "m68_skill_builder_sessions_v2"
down_revision = "m67_agents_runtime_profile"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    with op.batch_alter_table("skill_builder_sessions") as batch_op:
        ...
        batch_op.add_column(
            sa.Column("draft_workspace_path", sa.String(length=500), nullable=True)
        )
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("skill_builder_sessions") as batch_op:
        batch_op.drop_constraint(
            "fk_skill_builder_sessions_conversation", type_="foreignkey"
        )
