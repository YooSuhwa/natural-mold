...
async def test_e2e_post_killed_before_finalize_get_emits_stale(
    ...
    # 그대로. broker 는 mock finally 에서 close + ttl=0 으로 회수.
...
# `contextlib` import 사용 — `_make_executor_simulator` 가 close 책임을 항상
# 들고 있어 별도 cleanup 헬퍼는 필요 없지만 향후 시나리오 추가 시 broker /
# task leak 방어용으로 유지.
_ = contextlib
