...
``UI_DATA_TOOL_NAMES`` is empty (Phase 2 registers real tools), and the E2E demo
tool is recognized ONLY when ``demo_enabled`` is set by the caller (gated on
``e2e_scripted_model_enabled``). Real conversations therefore emit zero ui_data
events — the regression-zero guarantee — even if a real tool's name happened to
collide with the demo tool's.
"""
...
def ui_data_from_tool_result(
    ...
    *,
    ...
    additionally recognizes the E2E demo tool; otherwise only ``UI_DATA_TOOL_NAMES``
    (empty in Phase 1) is recognized — so production projects nothing.
