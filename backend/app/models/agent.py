...
class Agent(Base):
    ...
    # M71 additive policy storage. ``NULL`` means the exact pre-M71 runtime
    # graph and is resolved internally as the legacy-compatible policy.
    runtime_policy: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    opener_questions: Mapped[list[str] | None] = mapped_column(JSON, nullable=True, default=list)
    ...
    model_fallback_list: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
