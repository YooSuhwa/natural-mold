# Credential / Tools / Skills 그린필드 리라이트 (n8n 패턴 차용, 브랜딩 제거)
...
PoC 단계라 데이터 손실이 허용되므로, **n8n의 검증된 추상화(Cipher V2, ICredentialType, GenericAuth, OAuth2 helper, External Secrets, NodeParameters 동적 폼)를 Python/React로 포팅**하여 그린필드로 다시 짠다. 단, **결과물에 "n8n" 식별자·로고·라이선스 헤더는 일절 남기지 않는다**.

## 사용자 결정 사항 (확정)

1. **Cipher 포맷**: n8n 실제 알고리즘 차용 — 단일 블롭 `[version 1B][salt 32B][authTag 16B][ciphertext]` Base64 + HKDF-SHA256(info=`'moldy-encryption-v1'`)로 키/IV 도출. 키 식별은 별도 컬럼(`key_id`)으로.
...
**파일**:
- `scripts/check_branding.py` (신규) — 정규식 `\bn8n\b` 매치 0건 / `@n8n/*` 패키지 발견 시 실패 / 로고 SHA-256 블랙리스트
- `NOTICES.md` (신규) — 차용 알고리즘 출처 명기
...
**Cipher V2 정확한 스펙** (n8n core 차용, info 문자열만 자체화):
- KEY: 64-char hex (32 bytes)
- SALT: 32 bytes random
- IV: 12 bytes (HKDF 도출분의 마지막 12바이트)
- AUTH_TAG: 16 bytes
- HKDF info: `b'moldy-encryption-v1'` (n8n 문자열 절대 금지)
...
- `domain.py` — `CredentialDefinition` (n8n `ICredentialType` 포팅): `key, display_name, icon_id, properties, authenticate, test, pre_authentication, extends`
...
- `shared/icon.tsx` — Lucide + 자체 SVG. **n8n 아이콘 자산 사용 절대 금지.**
- `shared/empty-state.tsx`
- `shared/dynamic-fields-form.tsx` — **`FieldDef[]` → React 폼**. n8n NodeParameters 정보 구조 차용:
...
- 정규식 `\bn8n\b` 대소문자 무시, 소스 트리(`backend/`, `frontend/src`, `*.md` 일부) 매치 0건
- `pyproject.toml`, `package.json`에 `@n8n/*` 패키지 발견 시 실패
- `frontend/public`, `frontend/src/assets`의 SVG/PNG 파일 SHA-256이 블랙리스트와 일치 시 실패
- 예외 화이트리스트: `NOTICES.md` (출처 명기 목적)
...
| 리스크 | 대응 |
|---|---|
| 차용 코드 라이선스 (n8n SUL) | NOTICES.md에 출처 명기, 알고리즘만 차용·식별자/문자열/자산 모두 자체화. PoC 단계 + 사내 사용이라 SUL "경쟁 SaaS" 조항 비해당. 머지 전 변호사 1회 검토. |
| n8n 식별자 누락 | `scripts/check_branding.py` CI 게이트 + 머지 차단 |
...
### 수동 시나리오 (머지 전)
1. 빌드 산출물에서 `n8n` grep 0건. n8n 로고 자산 검색 결과 없음.
